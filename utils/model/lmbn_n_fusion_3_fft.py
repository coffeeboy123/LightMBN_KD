...
from .attention import BatchDrop, BatchFeatureErase_Top, PAM_Module, CAM_Module, SE_Module, Dual_Module, \
    BatchFeatureErase_Top_Bottom, BatchFeatureErase_Top_Bottom_Element
...
class LMBN_n_fusion_3_fft(nn.Module):
    def __init__(self, args):
        super(LMBN_n_fusion_3_fft, self).__init__()
        ...
        self.reduction_f_low = copy.deepcopy(reduction)
        ...
        self.batch_drop_block = BatchFeatureErase_Top_Bottom_Element(512, OSBlock)
    ...
    def forward(self, x):
        ...
        # 정규화 반경 격자
        yy = torch.linspace(-1, 1, Hf, device=fre.device).view(Hf, 1).expand(Hf, W2)
        xx = torch.linspace(0, 1, W2, device=fre.device).view(1, W2).expand(Hf, W2)
        rr = torch.sqrt(yy ** 2 + xx ** 2)  # (Hf,W2)

        # 2-band soft mask: low: [0,a), high: [a,∞)
        a = torch.clamp(self.falpha, 0.05, 1.2)
        low = 1.0 - torch.sigmoid(self.fk * (rr - a))
        high = torch.sigmoid(self.fk * (rr - a))
        ...
        f_low = torch.fft.irfft2(X_low, s=fre.shape[-2:], norm="ortho")
        f_high = torch.fft.irfft2(X_high, s=fre.shape[-2:], norm="ortho")
        ...
        fF_low = self.reduction_f_low(f_low)  # (cls, feat, ...)
        fF_high = self.reduction_f_high(f_high)

        ################

        fea = [f_glo[-1], f_glo_drop[-1], f_p0[-1]]

        return [fF_low[1], fF_high[1], f_glo_element[1], f_glo_bottom[1], f_glo[1], f_glo_drop[1], f_p0[1], f_p1[1],
                f_p2[1], f_p3[1], f_c0[1], f_c1[1]], fea, torch.stack(
            [fF_low[0], fF_high[0], f_glo[0], f_glo_drop[0], f_p0[0], f_p1[0], f_p2[0], f_p3[0], f_c0[0], f_c1[0]],
            dim=2)
